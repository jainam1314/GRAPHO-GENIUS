import os
from scripts.categorize import categorize
...
raw_feature_list_path = "C:\\Users\riddh\OneDrive\Desktop\BE PROJECT GIT CLONES\Handwriting-Analysis-using-Machine-Learning\scripts\Extracted_Features/raw_feature_list.txt"


if os.path.isfile(feature_list_path):
    print("Error: feature list already exists")

elif os.path.isfile(raw_feature_list_path):
    print("Info: raw feature list found")
    with open(raw_feature_list_path,'r') as raw_features, open(feature_list_path,'a') as features:
        for line in raw_features:
            ...
            raw_slant_angle = float(content[5])
            page_id = content[6]
